    ...
    <div class="list-container">
      ...
      <ul class="list-disc">
        <li *ngFor="let appointment of appointments$ | async" class="appointment-item">
          <span class="appointment-details">{{appointment.vehicleOwner}} - {{appointment.branch}} - {{appointment.time}} / {{appointment.date}}</span>
          <div class="button-group">
            <button (click)="editAppointment(appointment.id!)" class="icon-button edit-button" title="Edit">
            ...
            <button (click)="deleteAppointment(appointment.id!)" class="icon-button delete-button" title="Delete">
              <svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24">
